...
def parse(filename):
    ...
    if '\r' in f:
        if require_unix_line_endings:
            ...
            sys.stderr.write("%s:0: Warning: File contains DOS-style or Mac-style line endings." % filename)
...
def Warn(msg, *args):
    if args:
        msg = msg % args
    sys.stderr.write("%s:%d: Warning: %s" % (S.filename, S.line, msg))
...
def prologue(f):
    f.write("/* Autogenerated by %s on %s -- do not edit */\n" % (sys.argv[0],time.asctime()))
    f.write("""\
#include "rtapi.h"
#ifdef RTAPI
#include "rtapi_app.h"
...
def build_rt(tempdir, filename, mode, origfilename):
    ...
    makefile = os.path.join(tempdir, "Makefile")
    f = open(makefile, "w")
    f.write("obj-m += %s" % objname)
    f.write("include %s" % find_modinc())
    f.write("EXTRA_CFLAGS += -I%s" % os.path.abspath(os.path.dirname(origfilename)))
    f.write("EXTRA_CFLAGS += -I%s" % os.path.abspath('.'))
    f.close()
    if mode == INSTALL:
...
def document(filename, outfilename):
    ...
    if options.get("userspace"):
        ...
        if rest:
            ...
            if options.get("singleton") or options.get("count_function"):
                if has_personality:
                    f.write(".B loadrt %s personality=\\fIP\\fB\n" % comp_name)
                else:
                    f.write(".B loadrt %s\n" % comp_name)
            else:
                if has_personality:
                    f.write(".B loadrt %s [count=\\fIN\\fB|names=\\fIname1\\fB[,\\fIname2...\\fB]] [personality=\\fIP,P,...\\fB]\n" % comp_name)
                else:
                    f.write(".B loadrt %s [count=\\fIN\\fB|names=\\fIname1\\fB[,\\fIname2...\\fB]]\n" % comp_name)
            for type, name, default, doc in modparams:
                f.write("[%s=\\fIN\\fB]\n" % name)
            f.write("\n")
    ...
    if functions:
        f.write(".SH FUNCTIONS\n")
        for _, name, fp, doc in finddocs('funct'):
            f.write(".TP\n")
            f.write("\\fB%s\\fR" % to_hal_man(name))
            if fp:
    ...
    for _, name, type, array, dir, doc, value, personality in finddocs('pin'):
        f.write(lead + "\n")
        f.write(".B %s\\fR" % to_hal_man(name))
        f.write("%s %s\n" % (type, dir))
        if array:
            sz = name.count("#")
            if isinstance(array, tuple):
                ...
                f.write(" (%s=%0*d..%0*d)\n" % ("M" * sz, sz, 0, sz, array-1))
        if personality:
            f.write(" [if %s]\n" % personality)
        if value:
            f.write("\\fR(default: \\fI%s\\fR)\n" % value)
        else:
            f.write("\\fR\n")
        if doc:
            f.write("%s\n" % doc)
