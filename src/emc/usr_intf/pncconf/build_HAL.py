...
class HAL:
    ...
    def write_halfile(self, base):
        ...
        print("net spindle-manual-cw     halui.spindle.forward", file=file)
        print("net spindle-manual-ccw    halui.spindle.reverse", file=file)
        print("net spindle-manual-stop   halui.spindle.stop", file=file)
                ...
                if pinname:
                    ...
                    if self.d.externalmpg:
                        ...
                        if self.d.multimpg: # means MPG per axis
                            print("setp    axis.%d.jog-vel-mode 0" % axnum, file=file)
        ...
        if self.d.externalso:
            if self.d.so_usempg:
                ...
                print(file=file)
                print("    setp halui.spindle-override.direct-value false", file=file)
                print("    setp halui.spindle-override.scale .01", file=file)
                if pinname: # dedicated MPG
                    if self.d.findsignal("so-enable"): # make it enable-able externally
                        print("net so-enable             =>  halui.spindle-override.count-enable", file=file)
                    else:
                        print("    setp halui.spindle-override.count-enable true", file=file)
                    print("net so-count              =>  halui.spindle-override.counts", file=file)
                else: # shared MPG
                    print("net so-enable             =>  halui.spindle-override.count-enable", file=file)
                    print("net axis-selected-count  =>  halui.spindle-override.counts", file=file)
                print(file=file)
            elif self.d.so_useswitch:
                print("# connect spindle overide increments ", file=file)
                print(file=file)
                print("    setp halui.spindle-override.count-enable true", file=file)
                print("    setp halui.spindle-override.direct-value true", file=file)
                print("    setp halui.spindle-override.scale .01", file=file)
                print("net spindleoverride-incr  =>  halui.spindle-override.counts", file=file)
...
#*******************
    ...
    def connect_joint(self, file, jnum, let):
        ...
        if steppinname2:
            print("# ---PID for Tandem Joint---", file=file)
            self.build_pid(file, jnum, let+'2', stepflag)
            steppinname = steppinname2
            print("# Step Gen signals/setup for tandem axis stepper", file=file)
            print(file=file)
            print("setp   " + steppinname + ".dirsetup        [%s_%d]DIRSETUP"% (title, jnum), file=file)
            print("setp   " + steppinname + ".dirhold         [%s_%d]DIRHOLD"% (title, jnum), file=file)
            print("setp   " + steppinname + ".steplen         [%s_%d]STEPLEN"% (title, jnum), file=file)
            ...
            for i in stepinvertlist2:
                print("setp    "+i+".invert_output true", file=file)
            print(file=file)
            print("# ---Tandem stepper signals---", file=file)
            print(file=file)
            print("net %s-output                             => "% (let) + steppinname + ".velocity-cmd", file=file)
            print("net %s-enable                             => "% (let) + steppinname +".enable", file=file)
            print(file=file)
        ...
        if let =='s':
            ...
            print(file=file)
            print("net spindle-vel-cmd-rps        <=  motion.spindle-speed-out-rps", file=file)
            print("net spindle-vel-cmd-rps-abs    <=  motion.spindle-speed-out-rps-abs", file=file)
            print("net spindle-vel-cmd-rpm        <=  motion.spindle-speed-out", file=file)
            print("net spindle-vel-cmd-rpm-abs    <=  motion.spindle-speed-out-abs", file=file)
            print("net spindle-enable             <=  motion.spindle-on", file=file)
            ...
            print("net spindle-revs               =>  motion.spindle-revs", file=file)
            print("net spindle-at-speed           =>  motion.spindle-at-speed", file=file)
            print("net spindle-vel-fb-rps         =>  motion.spindle-speed-in", file=file)
            print("net spindle-index-enable      <=>  motion.spindle-index-enable", file=file)
            print(file=file)
                ...
                if not p == "unused-output":
                    ...
                    if "parport" in pinname:
                        ...
                        # mainboard GPIOO require extra setup commands
                            ...
                            print("setp %s true"% (pinname + ".is_output"), file=file)
